...
Usage
-----
A transpiled, [UMD](https://github.com/umdjs/umd) version of **ES6 Rest Client** is provided in the `dist` folder for testing purposes; projects meant for production environments should consume the library via ES6-aware bundling tools like [Rollup](https://github.com/rollup/rollup).
...
| :--    | :--         | :--       | :--           |
| `<any>` | `yes` | `RestClient` | Any property getter invoked on the client instance causes an equally named route fragment to be appended to the base URI specified in the [settings object](#settings-object). |
...
**Note:** invoking a non-existent method on the **ES6 Rest Client** causes its name to be parsed as route fragment and the `get()` method to be implicitly called with any arguments given in the former (see the [example](#es6-rest-client-beta) above).

### Settings object ###

|*key*|*default*|*description*|
| :-- | :--     | :--         |
| `method?:String` | `'GET'` | Default HTTP verb. |
| `baseURI?:String` | `'/'` | Remote resource base URI. |
...
### Creating and exporting a third-party API client ###
...
### Fetching a resource upon successful existence check ###
...
**Note:** you may want to check your browser [ECMAScript 6 capabilities](#required-es6-features) first. At time of writing, the only browser with full support for the required ECMAScript 6 features is [Firefox (Developer Edition)](https://www.mozilla.org/en-US/firefox/developer/).
